                 ...
                 const ArgsInfo& args_info)
{
  ...
  std::string compiler_path = args[0];
  Stat st = Stat::stat(compiler_path, Stat::OnError::log);
#ifdef _WIN32
  if (!st) {
    compiler_path = Win32Util::add_exe_suffix(args[0]);
    st = Stat::stat(compiler_path, Stat::OnError::log);
  }
#endif
